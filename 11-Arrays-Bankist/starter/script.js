...
// weight in kg, result in g
const recommendedFood = weight => Math.floor(weight ** 0.75 * 28);
const checkEating = dog => (dog.curFood > dog.recFood ? 'much' : 'little');
const checkEatingOK = dog =>
  dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1;
const checkEatingExact = dog => dog.curFood === dog.recFood;
...
console.log(`sarahDog eats too ${checkEating(sarahDog)}`);
...
// 6.
console.log(dogs.every(dog => checkEatingOK(dog)));

// 7.
const dogsEatingOK = dogs.filter(dog => checkEatingOK(dog));
console.log(dogsEatingOK);

// 8.
const dogsGroupedByEating = Object.groupBy(dogs, dog => {
  if (checkEatingExact(dog)) return 'exact';
  else return 'too-' + checkEating(dog);
});
console.log(dogsGroupedByEating);

// 9.
const dogsGroupedByNumOwner = Object.groupBy(
  dogs,
  dog => `${dog.owners.length}-owners`,
);
console.log(dogsGroupedByNumOwner);

// 10.
const dogsSortByAscRecFood = dogs.toSorted((a, b) => a.recFood - b.recFood);
console.log(dogsSortByAscRecFood);

const dogsSortByDescRecFood = dogs.toSorted((a, b) => b.recFood - a.recFood);
console.log(dogsSortByDescRecFood);
